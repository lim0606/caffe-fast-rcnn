...
DEFINE_string(target_blob, "loss3/argmax",
...
DEFINE_string(target_layer, "loss3/argmax",
    "The name of blob you want to print out.");
...
//    if ("BN" == layers[i]->layer_param().type()) {
...
//  LOG(INFO) << "# of imgs (read): " << img_idx << ", # of imgs (processed): " << img_processed_idx;
  ...
  LOG(INFO) << "target_layer (to be printed): " << FLAGS_target_blob;
  LOG(INFO) << "target_blob (to be printed): " << FLAGS_target_blob;
  int img_idx = 0, img_processed_idx = 0;
  for (int i = 0; i < iterations; ++i) {
    //printf("iter: %d\n", i);
    caffe_test_net.ForwardPrefilled();
    const Layer<float>* argmax_layer = CHECK_NOTNULL(caffe_test_net.layer_by_name(FLAGS_target_layer).get());
    const Blob<float>* argmax = CHECK_NOTNULL(caffe_test_net.blob_by_name(FLAGS_target_blob).get());
    caffe::ArgMaxParameter argmax_param = argmax_layer->layer_param().argmax_param();
    bool output_max_val = argmax_param.out_max_val();
    int top_k = argmax_param.top_k();
    int batchsize = argmax->shape(0);

    //LOG(INFO) << "output_max_val: " << argmax_param.out_max_val(); 
    //LOG(INFO) << "top_k: " << argmax_param.top_k(); 
    //LOG(INFO) << "argmax shape: " << argmax->shape(0) << ", " 
    //                              << argmax->shape(1) << ", "
    //                              << argmax->shape(2) << ", " 
    //                              << argmax->shape(3); 
 
    const float* argmax_vec = argmax->cpu_data();
   
    for (int j = 0; j < batchsize; ++j){
      vector<int> predict_cls, predict_conf; // classe, confidence
      predict_cls.resize(0); predict_conf.resize(0); 
      if (img_idx < numdata) {
        predict_cls.push_back(static_cast<int>(argmax_vec[j*top_k*(1+output_max_val)+0]));
        fprintf(prediction_file, "%d", predict_cls[0]+1);
        for (int k = 1; k < top_k; ++k) {
          predict_cls.push_back(static_cast<int>(argmax_vec[j*top_k*(1+output_max_val)+k]));
          fprintf(prediction_file, " %d", predict_cls[k]+1);
        }
        fprintf(prediction_file, "\n");
 
        if (output_max_val) {
          predict_conf.push_back(static_cast<int>(argmax_vec[j*top_k*(1+output_max_val)+top_k+0]));
          for (int k = 1; k < top_k; ++k) {
            predict_cls.push_back(argmax_vec[j*top_k*(1+output_max_val)+top_k+k]);
          }
        }
        ++img_processed_idx;
      }
