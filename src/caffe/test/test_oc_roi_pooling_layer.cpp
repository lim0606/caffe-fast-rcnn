...
class OCROIPoolingLayerTest : public MultiDeviceTest<TypeParam> {
  typedef typename TypeParam::Dtype Dtype;

 protected:
  OCROIPoolingLayerTest()
      : blob_bottom_data_(new Blob<Dtype>(4, 3, 12, 8)), 
        blob_bottom_rois_(new Blob<Dtype>(4, 5, 1, 1)),
        blob_top_inward_data_(new Blob<Dtype>()),
        blob_top_outward_data_(new Blob<Dtype>()) {
    // fill the values
    ...
    blob_top_vec_.push_back(blob_top_inward_data_);
    blob_top_vec_.push_back(blob_top_outward_data_);
  }
  virtual ~OCROIPoolingLayerTest() {
    delete blob_bottom_data_;
    delete blob_bottom_rois_;
    delete blob_top_inward_data_;
    delete blob_top_outward_data_; 
  }
  Blob<Dtype>* const blob_bottom_data_;
  Blob<Dtype>* const blob_bottom_rois_;
  Blob<Dtype>* const blob_top_inward_data_;
  Blob<Dtype>* const blob_top_outward_data_;  
  vector<Blob<Dtype>*> blob_bottom_vec_;
  vector<Blob<Dtype>*> blob_top_vec_;
};

TYPED_TEST_CASE(OCROIPoolingLayerTest, TestDtypesGPU);

TYPED_TEST(OCROIPoolingLayerTest, TestGradient) {
  typedef typename TypeParam::Dtype Dtype;
  LayerParameter layer_param;
  OCROIPoolingParameter* oc_roi_pooling_param =
      layer_param.mutable_oc_roi_pooling_param();
  oc_roi_pooling_param->set_pooled_h(4/*6*/);
  oc_roi_pooling_param->set_pooled_w(4/*6*/);
  oc_roi_pooling_param->set_margin_h(1);
  oc_roi_pooling_param->set_margin_w(1);  
  OCROIPoolingLayer<Dtype> layer(layer_param);
  GradientChecker<Dtype> checker(1e-4, 1e-2);
  //layer.SetUp(this->blob_bottom_vec_, this->blob_top_vec_);  
  //for (int i=0; i<40; ++i){ 
  //  checker.CheckGradientSingle(&layer, this->blob_bottom_vec_, this->blob_top_vec_, 0, 1, i);
  //}  // PASSED
  //checker.CheckGradientSingle(&layer, this->blob_bottom_vec_, this->blob_top_vec_, 0, 0, 0); // PASSED
