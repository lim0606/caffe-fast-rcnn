/**
 * modified from https://github.com/jeffdonahue/caffe/blob/recurrent/src/caffe/layers/lstm_unit_layer.cpp
 */ 
#include <algorithm>
#include <cmath>
#include <vector>
...
void LSTMUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  CHECK_EQ(bottom.size(), 2) << "bottom.size() should equal to 2"; 
  //const int num_instances = bottom[0]->shape(1);
  const int num_instances = bottom[0]->shape(0); // num batch
...
void LSTMUnitLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  const int num = bottom[0]->shape(/*1*/0);
